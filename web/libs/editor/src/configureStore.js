...
export const configureStore = async (params, events) => {
  ...
  if (!params?.config && env.getExample) {
    ...
    params.reviewers = params.task.reviewers ? getAssigneeInfo(params.task.reviewers) : [];
    params.annotators = params.task.annotators ? getAssigneeInfo(params.task.annotators) : [];
  }
  if (params.task?.id) {
