...
import {CustomJSON} from "../../../datamanager/src/stores/types";
...
export default types
  .model('AppStore', {
    ...
    reviewers: types.optional(types.array(CustomJSON), []),
    ...
    */
    ...
    isSubmitting: false,
    /**
     * Flag for disable task in Labelo
     */
  ...
  .actions(self => {
    ...
    function acceptAnnotation(mode="Accept") {
      if (self.isSubmitting) return;
      ...
      handleSubmittingFlag(async () => {
        ...
        entity.dropDraft();
        await getEnv(self).events.invoke('acceptAnnotation', self, { isDirty, entity, mode });
        entity?.setReviewStatus(mode)
        self.incrementQueuePosition();
    ...
    function rejectAnnotation({ comment = null }) {
      ...
      handleSubmittingFlag(async () => {
        ...
        entity.dropDraft();
        await getEnv(self).events.invoke('rejectAnnotation', self, { isDirty, entity, comment, mode: "Reject" });
        entity?.setReviewStatus("Reject")
