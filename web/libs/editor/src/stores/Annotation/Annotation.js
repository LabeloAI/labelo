...
export const Annotation = types
  .model('Annotation', {
    ...
    review_status: types.maybeNull(types.string),
    reject_comment_resolved: types.boolean,
    reject_comment: types.maybeNull(types.integer),
  ...
  .preProcessSnapshot(sn => {
    ...
    const rejectComment = sn.comments?.find(item => item.id === sn.reject_comment)
    return {
      ...sn,
      ...
      reject_comment_resolved: rejectComment?.is_resolved || false
    };
  })
  ...
  .actions(self => ({
    ...
    updateRejectCommentResolve: flow(function *(id) {
      if(self.reject_comment === id){
        self.reject_comment_resolved = true
      }
    }),

    setReviewStatus: flow(function *(status) {
