...
interface Comment {
  ...
  setCommentId: (comment: object) => void;
  updateRejectCommentResolve: (id: number) => void;
}
              ...
              <Dropdown.Trigger
                content={(
                  <Menu size="auto">
                    <Menu.Item onClick={() => {
                      if(!resolved) {
                        updateRejectCommentResolve(id)
                      }
                      toggleResolve()
                    }}>{resolved ? 'Unresolve' : 'Resolve'}</Menu.Item>
