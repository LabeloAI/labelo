...
export const panelDefault: Record<string, PanelBBox> = {
...
export const panelReviewer: Record<string, PanelBBox> = {
  'comments-info-history': {
    order: 1,
    top: 0,
    left: 0,
    relativeLeft: 0,
    ...
    alignment: Side.right,
    maxHeight: DEFAULT_PANEL_MAX_HEIGHT,
    panelViews:  [panelViews[4], panelViews[3], panelViews[1]],
    reviewer: true
  },
  'regions-relations': {
    order: 2,
...
export const restorePanel = (isReviewer: boolean): StoredPanelState => {
  ...
  const countOfAllAvailableTabs = panelViews.length;
  ...
  if (!allTabs || allTabs.length !== countOfAllAvailableTabs) {
    if(isReviewer){
      return { panelData: panelReviewer, collapsedSide: defaultCollapsedSide };
    }
    return { panelData: panelDefault, collapsedSide: defaultCollapsedSide };
  }
