...
export class LSFWrapper {
  ...
  constructor(dm, element, options) {
    ...
    if (this.project.require_comments_on_skip === true) {
  ...
  onAcceptAnnotation = async (store, data) => {
    let annotationID = data.entity?.pk;
    data.user = window.APP_SETTINGS.user.id;
    await this.datamanager.apiCall("acceptAnnotation", {annotationID}, data);
  };

  onRejectAnnotation = async (store, data) => {
    let annotationID = data.entity?.pk;
    data.user = window.APP_SETTINGS.user.id;
    await this.datamanager.apiCall("rejectAnnotation", {annotationID}, data);
  };
