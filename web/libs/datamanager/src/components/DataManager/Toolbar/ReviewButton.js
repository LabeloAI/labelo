...
const injector = inject(({ store }) => {
  ...
  return {
    ...
    canReview: false, //TODO: add logic for review
  };
});

export const ReviewButton = injector(
  ({ store, canLabel, size, target, selectedCount, disabled, canReview }) => {
    ...
    const onReviewAll = () => {
      localStorage.setItem('dm:reviewstream:mode', 'all');
      store.startReviewStream();
    };

    const onReviewVisible = () => {
      localStorage.setItem('dm:reviewstream:mode', 'filtered');
        ...
        <div>
          <div style={{ display: 'flex' }}>
            <Button
              ...
              onClick={onReviewAll}
            >
              Review {selectedCount ? selectedCount : 'All'} Task
          ...
          <Button
            ...
            onClick={onReviewVisible}
          >
            Review Tasks As Displayed
          </Button>
        </div>
      </Interface>
    ) : null;
