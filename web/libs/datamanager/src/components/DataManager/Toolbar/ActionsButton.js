...
import { AssignAnnotatorModal } from "./AssignAnnotatorModal";
...
const injector = inject(({ store }) => ({
  ...
  selectedTaskIds: store.currentView?.selected?.items?.map((item) => item.id) ?? [],
}));
...
export const ActionsButton = injector(
  observer(({ store, size, hasSelected, ...rest }) => {
    ...
    const [isAnnotatorModalOpen, setIsAnnotatorModalOpen] = useState(false);
    const [isReviewerModalOpen, setIsReviewerModalOpen] = useState(false);
    const projectId = store.project.id;
    const selectedItems = store.currentSelection
    const taskId = store.currentSelection.included;
    const taskEx = store.currentSelection.excluded;
    ...
    const currentUserRole = currentUser.group;
    ...
    const invokeAction = (action, destructive) => {
      ...
      } else if (action.id === "assign_annotators") {
        setIsAnnotatorModalOpen(true);
      } else if (action.id === "assign_reviewer") {
    ...
    const isUserRestricted = useMemo(() => {
      return currentUserRole === "reviewer" || currentUserRole === "annotater";
    },[currentUserRole]);
    ...
    return (
      ...
      {!isUserRestricted && (
        ...
        {isAnnotatorModalOpen && (
          <AssignAnnotatorModal
            isOpen={isAnnotatorModalOpen}
            onClose={() => setIsAnnotatorModalOpen(false)}
            projectId={projectId}
