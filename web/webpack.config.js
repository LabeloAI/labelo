...
module.exports = composePlugins(withNx({
  ...
  config.module.rules.forEach((rule) => {
        ...
        l.forEach(_l => {
          _l.options = {
            ...
            stylusOptions: {
              ...
              import: [path.resolve(__dirname, 'apps/labeloapp/src/themes/default/variables.styl')],
            },
          };
        });
      });
    ...
    if (rule.test.toString().match(/scss|sass/) && !isCssModule) {
      const r = rule.oneOf.filter((r) => {
        // Exclude rules without loaders
        if (!r.use) return false;
    
        const testString = r.test.toString();
    
        // Exclude CSS modules as they don't need prefixing
        if (testString.match(/module/)) return false;
    
        // Target SCSS/SASS files with 'css-loader' included
