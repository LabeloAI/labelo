...
ENTRYPOINT_PATH=/labelo/deploy/docker-entrypoint.d
...
if [ "$1" = "labelo-uwsgi" ]; then
  exec_entrypoint "$ENTRYPOINT_PATH/app/"
  exec_or_wrap_n_exec uwsgi --ini /labelo/deploy/uwsgi.ini
elif [ "$1" = "labelo-migrate" ]; then
  exec_entrypoint "$ENTRYPOINT_PATH/app-init/"
  exec python3 /labelo/manage.py locked_migrate >&3
else
  exec_or_wrap_n_exec "$@"
fi
